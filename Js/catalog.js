...
function displayCatalogItems(items) {
  ...
  items.forEach((item) => {
    ...
    addToCompareButton.textContent = "Добавить в сравнение";
...
document.getElementById('ok_button').addEventListener('click', function() {
  ...
  if (currentCore !== null) {
    catalogItemNow = catalogItem.filter(item => item.core === currentCore);
  } else {
    catalogItemNow = [...catalogItem];
  }
 ...
 document.querySelector('#closeButton').addEventListener('click', function() {
  document.querySelector('.filter_items').classList.remove('show');
...
function addToComparisonList(item) {
  ...
  if (addedItems.includes(item)) {
    showNotification({content: "Этот процессор уже добавлен в сравнение.", type: "info"});
    return;
  } 
  if (rows >= 5) {
    showNotification({content: "Вы можете добавить не более 5 процессоров для сравнения!", type: "info"});
    return;
  }
 ...
 document.querySelectorAll('.product-container').forEach((product, index) => {
  product.addEventListener('click', function() {
    ...
    <img src="${catalogItem[index].image}" alt="${catalogItem[index].name}">
     ...
     <p>Производитель: ${catalogItem[index].category}</p>
     <p>Дата выпуска: ${catalogItem[index].date}</p>
     <p>Архитектура: ${catalogItem[index].architect}</p>
     <p>Техпроцесс: ${catalogItem[index].techprocess}</p>
     <p>Число ядер: ${catalogItem[index].core}</p>
     <p>Число потоков: ${catalogItem[index].threads}</p>
     <p>Тактовая частота: ${catalogItem[index].clock}</p>
     <p>Встроенный видеоускоритель: ${catalogItem[index].IGP}</p>
     <p>Потребляемая мощность: ${catalogItem[index].TDP}</p>
     <p>Средняя цена: ${catalogItem[index].price}</p>
   `;
